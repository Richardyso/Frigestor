...
/** Conta criada por administrador — acesso ja liberado. */
function buildContaCriadaPeloAdminEmail(nome) {
  const nomeCurto = primeiroNome(nome);
  const urlLogin = loginUrl();
  ...
  const texto = [
    `Ola, ${nomeCurto}!`,
    '',
    'Seja bem-vindo(a) a plataforma Frigestor.',
    '',
    'Seu administrador criou seu acesso. Voce ja pode entrar com o e-mail e a senha informados no cadastro.',
    '',
    'Acesse em:',
    urlLogin,
    '',
  ...
  const html = layoutEmail(`
    <p style="margin:0 0 16px;">Ola, <strong>${nomeCurto}</strong>!</p>
    <p style="margin:0 0 16px;">Seja bem-vindo(a) a <strong>plataforma Frigestor</strong> — gestao de equipamentos, visitas tecnicas e QR Code.</p>
    <p style="margin:0 0 16px;">Seu administrador criou seu acesso. <strong>Voce ja pode entrar</strong> com o e-mail e a senha definidos no cadastro.</p>
    ${botaoLogin(urlLogin)}
    <p style="margin:0;font-size:14px;color:#6c7e97;">Duvidas? Fale com o administrador da sua empresa.</p>
  `, urlLogin);

  return {
    subject: 'Bem-vindo a plataforma Frigestor',
    text: texto,
    html
  };
}

async function sendContaCriadaPeloAdminEmail(destinatario, nome) {
  const mail = buildContaCriadaPeloAdminEmail(nome);
  const result = await enviarEmail(destinatario, mail);
