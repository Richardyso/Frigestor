...
/** Conta criada por administrador — acesso ja liberado (credenciais em texto claro). */
function buildContaCriadaPeloAdminEmail(nome, emailLogin, senhaPlana) {
  const nomeCurto = primeiroNome(nome);
  const login = String(emailLogin || '').trim();
  const senha = String(senhaPlana || '');
  const urlLogin = loginUrl();

  const credenciaisHtml = `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px;background:#f4f7fb;border:1px solid #e0e6f0;border-radius:10px;">
      <tr>
        <td style="padding:18px 20px;">
          <p style="margin:0 0 12px;font-size:13px;font-weight:700;color:#0e3d7a;text-transform:uppercase;letter-spacing:0.04em;">Seus dados de acesso</p>
          <p style="margin:0 0 10px;font-size:15px;line-height:1.5;color:#243247;"><strong>Login (usuario):</strong><br/><span style="color:#0a1626;">${escHtml(login)}</span></p>
          <p style="margin:0;font-size:15px;line-height:1.5;color:#243247;"><strong>Senha:</strong><br/><span style="font-family:Consolas,Monaco,monospace;font-size:16px;color:#0a1626;">${escHtml(senha)}</span></p>
        </td>
      </tr>
    </table>`;
  ...
  const texto = [
    `Ola, ${nomeCurto}!`,
    '',
    'Seu administrador criou seu acesso no Frigestor (gestao de equipamentos, visitas tecnicas e QR Code).',
    'Voce ja pode entrar — nao precisa aguardar aprovacao.',
    '',
    'COMO ENTRAR',
    '1. Abra o link de login abaixo no celular ou no computador.',
    '2. Na tela de login, preencha exatamente:',
    '',
    `   Login (usuario): ${login}`,
    '   (O login e sempre o seu proprio e-mail cadastrado.)',
    '',
    `   Senha: ${senha}`,
    '   (Esta e a senha definida pelo administrador. Use-a diretamente — nao e obrigatorio trocar no primeiro acesso.)',
    '',
    'Link de acesso:',
    urlLogin,
    '',
    'Duvidas ou esqueceu a senha? Fale com o administrador da sua empresa.',
    '',
    'Atenciosamente,',
    'Equipe Frigestor'
  ].join('\n');
  ...
  const html = layoutEmail(`
    <p style="margin:0 0 16px;">Ola, <strong>${escHtml(nomeCurto)}</strong>!</p>
    <p style="margin:0 0 16px;">Seu administrador criou seu acesso no <strong>Frigestor</strong> — gestao de equipamentos, visitas tecnicas e QR Code. <strong>Voce ja pode entrar</strong>; nao precisa aguardar aprovacao.</p>
    <p style="margin:0 0 12px;font-size:15px;color:#243247;"><strong>Como entrar:</strong></p>
    <ol style="margin:0 0 16px;padding-left:22px;font-size:15px;line-height:1.65;color:#243247;">
      <li style="margin-bottom:8px;">Abra o link abaixo no celular ou no computador.</li>
      <li>Na tela de login, use o <strong>e-mail</strong> como usuario e a <strong>senha</strong> abaixo (a mesma que o administrador cadastrou).</li>
    </ol>
    ${credenciaisHtml}
    <p style="margin:0 0 16px;font-size:14px;color:#6c7e97;">O campo de login aceita somente o seu e-mail. A senha acima e a que foi definida no cadastro — <strong>nao e obrigatorio altera-la no primeiro acesso</strong>.</p>
    ${botaoLogin(urlLogin)}
    <p style="margin:0;font-size:14px;color:#6c7e97;">Duvidas? Fale com o administrador da sua empresa.</p>
  `, urlLogin);

  return {
    subject: 'Seu acesso ao Frigestor — dados de login',
    text: texto,
    html
  };
}

async function sendContaCriadaPeloAdminEmail(destinatario, nome, emailLogin, senhaPlana) {
  const mail = buildContaCriadaPeloAdminEmail(nome, emailLogin, senhaPlana);
  const result = await enviarEmail(destinatario, mail);
  if (result.ok) console.log(`[email] Boas-vindas (conta admin) enviado para ${destinatario}`);
  return result;
}

/** Nova empresa cadastrada na plataforma — admin ja ativo + guia de primeiros passos. */
function buildEmpresaCadastroAtivoEmail(nomeAdmin, emailLogin, senhaPlana, nomeEmpresa) {
  const nomeCurto = primeiroNome(nomeAdmin);
  const login = String(emailLogin || '').trim();
  const senha = String(senhaPlana || '');
  ...
  const credenciaisHtml = `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px;background:#f4f7fb;border:1px solid #e0e6f0;border-radius:10px;">
      <tr>
        <td style="padding:18px 20px;">
          <p style="margin:0 0 12px;font-size:13px;font-weight:700;color:#0e3d7a;text-transform:uppercase;letter-spacing:0.04em;">Seus dados de acesso</p>
          <p style="margin:0 0 10px;font-size:15px;line-height:1.5;color:#243247;"><strong>Login (e-mail):</strong><br/><span style="color:#0a1626;">${escHtml(login)}</span></p>
          <p style="margin:0;font-size:15px;line-height:1.5;color:#243247;"><strong>Senha:</strong><br/><span style="font-family:Consolas,Monaco,monospace;font-size:16px;color:#0a1626;">${escHtml(senha)}</span></p>
        </td>
      </tr>
    </table>`;
  ...
  const passosHtml = `
    <ol style="margin:0 0 20px;padding-left:22px;font-size:15px;line-height:1.65;color:#243247;">
      <li style="margin-bottom:10px;"><strong>Entre no painel</strong> com o botao abaixo (e-mail e senha acima).</li>
      <li style="margin-bottom:10px;"><strong>Cadastre seus tecnicos</strong> — eles registram visitas e equipamentos em campo.</li>
      <li style="margin-bottom:10px;"><strong>Cadastre clientes e equipamentos</strong> (maquinas de ar, split, etc.) vinculados a cada cliente.</li>
      <li style="margin-bottom:10px;"><strong>Registre visitas tecnicas</strong> — manutencoes, instalacoes e historico de cada equipamento.</li>
      <li><strong>Gere o QR Code</strong> de cada equipamento. Ao escanear, o cliente ve o historico completo daquela maquina.</li>
    </ol>`;

  const texto = [
    `Ola, ${nomeCurto}!`,
    '',
    `Sua empresa ${empresa} foi cadastrada no Frigestor.`,
    'Seu cadastro ja esta ativo — voce ja pode entrar no sistema.',
    '',
    'DADOS DE ACESSO',
    `Login: ${login}`,
    `Senha: ${senha}`,
    ...
    '2. Cadastre tecnicos da equipe.',
    '3. Cadastre clientes e equipamentos (maquinas).',
    '4. Registre visitas tecnicas.',
    '5. Gere o QR Code de cada equipamento — o cliente escaneia e ve o historico da maquina.',
    '',
    'Link de acesso:',
    urlLogin,
    '',
  ...
  const html = layoutEmail(`
    <p style="margin:0 0 16px;">Ola, <strong>${escHtml(nomeCurto)}</strong>!</p>
    <p style="margin:0 0 16px;">A empresa <strong>${escHtml(empresa)}</strong> foi cadastrada no <strong>Frigestor</strong>.</p>
    <p style="margin:0 0 20px;padding:14px 16px;background:#e8f4fc;border-left:4px solid #1e5bb8;border-radius:6px;font-size:15px;color:#0e3d7a;"><strong>Seu cadastro ja esta ativo.</strong> Voce ja pode entrar no painel administrativo.</p>
    ${credenciaisHtml}
    <p style="margin:0 0 12px;font-size:15px;color:#243247;"><strong>Como comecar:</strong></p>
    ${passosHtml}
    ${botaoLogin(urlLogin)}
    <p style="margin:0;font-size:14px;color:#6c7e97;">Duvidas? Responda este e-mail ou fale conosco.</p>
  `, urlLogin);

  return {
    subject: `Bem-vindo ao Frigestor — ${empresa} ja esta ativa`,
    text: texto,
    html
  };
}

async function sendEmpresaCadastroAtivoEmail(destinatario, nomeAdmin, emailLogin, senhaPlana, nomeEmpresa) {
  const mail = buildEmpresaCadastroAtivoEmail(nomeAdmin, emailLogin, senhaPlana, nomeEmpresa);
  const result = await enviarEmail(destinatario, mail);
  if (result.ok) console.log(`[email] Boas-vindas (nova empresa) enviado para ${destinatario}`);
  return result;
}
