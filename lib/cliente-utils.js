...
function normalizarEmails(val) {
  if (!val) return [];
  const lista = Array.isArray(val) ? val : normalizarListaTexto(val);
  return lista.map((item) => {
    if (typeof item === 'object' && item !== null) {
      const email = String(item.email || '').trim().toLowerCase();
      const responsavel = String(item.responsavel || item.rotulo || '').trim();
      if (!email && !responsavel) return null;
      return { email, responsavel };
    }
    const s = String(item || '').trim();
    if (!s) return null;
    const sep = s.match(/^(.+?)\s*[—\-|]\s*(.+)$/);
    if (sep) {
      return {
        email: sep[1].trim().toLowerCase(),
        responsavel: sep[2].trim()
      };
    }
    if (s.includes('@')) return { email: s.toLowerCase(), responsavel: '' };
    return { email: '', responsavel: s };
  }).filter(Boolean);
}

function validarEmailsLista(emails) {
  const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  for (const item of emails || []) {
    if (item.email && !re.test(item.email)) {
      return 'Informe um e-mail valido ou deixe o campo vazio.';
    }
  }
  return null;
}
...
function normalizarClientePayload(body, existente = null) {
  ...
  const emails = 'emails' in (body || {})
    ? normalizarEmails(body.emails)
    : (existente?.emails || []);
